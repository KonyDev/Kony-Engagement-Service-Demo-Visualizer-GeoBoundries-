...
function updateAudience() {
    ...
    var payload1 = '{' + '"ksid":' + ksid + ',' + '"lastName" :' + '\"' + audienceLastName + '\",' + '"email" : ' + '\"' + audienceEmail + '\",' + '"active" :' + audienceStatus + ',' + '"firstName" :' + '\"' + audienceFirstName + '\",' + '"mobileNumber" :' + '\"' + audienceMob + '\",' + '"smsSubscription" :' + audienceSmsSubs + ',' + '"emailSubscription":' + audienceEmailSubs + ',' + '"country":' + '\"' + audienceCountry + '\",' + '"state":' + '\"' + audienceState + '\",' + '"pushSubscription":' + audiencePushSubs + '}';
    kony.print("\npayload1:-" + payload1);
    var inputParamTable = {
        httpheaders: {
...
function updatePushSubscription() {
        ...
        if (status == 400) {
            ...
            if (result["subscriptionResponse"] != undefined) {
                if (result["subscriptionResponse"]["statusCode"] == 200) {
                    ...
                    isPushSubs = true;
                    kony.application.dismissLoadingScreen();
                    if (kony.os.deviceInfo().name == "android") updateAudience();
                    else updateAudience();
    ...
    try {
        ...
        var connHandle = kony.net.invokeServiceAsync(pushSubscUrl, inputParamTable, asyncCallback);
    } catch (err) {
